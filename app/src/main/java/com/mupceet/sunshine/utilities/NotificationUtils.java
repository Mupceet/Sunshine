...
import com.mupceet.sunshine.DetailActivity;
...
public class NotificationUtils {
    ...
    private static final String WEATHER_NOTIFICATION_CHANNEL_ID = "Weather";
...
//          COMPLETED (2) Use NotificationCompat.Builder to begin building the notification
            initNotificationChannelIfNeed(context);
...
//          COMPLETED (8) Save the time at which the notification occurred using SunshinePreferences
    ...
    private static void initNotificationChannelIfNeed(Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            return;
        }
        NotificationManager manager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        if (Objects.requireNonNull(manager).getNotificationChannel(WEATHER_NOTIFICATION_CHANNEL_ID) != null) {
            return;
        }
